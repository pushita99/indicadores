		...
		<div class="col-12">
			<div class="card">
				...
				<Dialog v-model:visible="userDialog" :style="{width: '450px'}" header="Inserte los datos del usuario"
					...
					<div class="field">
						<label for="roleId" class="mb-3">Rol</label>
						<AutoComplete placeholder="Buscar rol" id="rolId" :dropdown="true"  v-model="rol" :suggestions="autoFilteredRoles" @complete="searchRoles($event)" :force-selection="true" field="name"/>
					</div>
					
					<div class="field">
						<label for="directionId" class="mb-3">Dirección</label>
						<AutoComplete placeholder="Buscar dirección" id="direcciónId" :dropdown="true"  v-model="direction" :suggestions="autoFilteredDirections" @complete="searchDirections($event)" :force-selection="true" field="name"/>
					</div>
				</Dialog>
			...
			if (this.user.name.trim()) {
				if (this.user.id) {
					...
					try {
						...
						this.$toast.add({ severity: 'success', summary: 'Successful', detail: 'Usuario insertado con éxito', life: 3000 });
						this.loadData();
					} catch (error) {
						this.$toast.add({ severity: 'error', summary: 'Error', detail: error, life: 3000 });
      					console.log(error);				
		...
		searchRoles(event) {
			setTimeout(() => {
				if (!event.query.trim().length) {
					...
					this.autoFilteredRoles = this.roles.filter((rol) => {
