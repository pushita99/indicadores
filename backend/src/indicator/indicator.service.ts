...
export class IndicatorService {
  ...
  async update(id: number, updateIndicatorDto: UpdateIndicatorDto) {    
    ...
    const { id: dtoId, ...data} = updateIndicatorDto;
    
    const indicator = await this.indicatorRepository.preload ({
      ...updateIndicatorDto,
      measurementunit: {id:updateIndicatorDto.measurementunit}
    });

    if (!indicator) throw new NotFoundException('Empresa no encontrada')
